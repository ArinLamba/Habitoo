import { formatDate, formatDisplayDate, parseLocalDate } from "./date";
import {
  HABIT_STATUS,
  type Completion,
  type Habit,
...
const getPeriodKey = (frequency: Habit["frequency"], date: Date) => {
...
export const buildHabitStats = (habit: Habit, completions: Completion[]) => {
  ...
  for (const date of getDates(createdDate, today)) {
    ...
    if (calendar.skipped.has(dateStr) || calendar.failed.has(dateStr)) {
      continue;
    }

    if (isHabitCompletedForDate(habit, habitCompletions, dateStr)) {
  ...
  } = getHabitPeriodStreaks(habit, habitCompletions, todayStr);
  ...
  const getDateValue = (date: string) => {
    if (calendar.skipped.has(date) || calendar.failed.has(date)) {
      return 0;
    }
  ...
  const getRangeValue = (dates: Date[]) => {
    return dates.reduce((total, date) => total + getDateValue(formatDate(date)), 0);
  };
...
export const buildStats = (habits: Habit[], completions: Completion[]) => {
  ...
  completions.forEach((completion) => {
    if (completion.date > todayStr) return;
    ...
    if (!isHabitCompletedForDate(habit, completions, completion.date)) {
      return;
    }

    completedPairs.add(key);
    doneSet.add(completion.date);
    dayCount.set(completion.date, (dayCount.get(completion.date) || 0) + 1);

    const day = parseLocalDate(completion.date).getDay();
    weekdayCount[day]++;
  });
  ...
  const habitStatsMap = new Map(habitStats.map((habit) => [habit.habitId, habit]));
  ...
  const consistencySet = new Set(
    completions
      ...
      .map((completion) => `${completion.habitId}-${completion.date}`)
  );
  ...
  const consistency =
    totalDays > 0
      ? Math.min(100, Math.round((consistencySet.size / totalDays) * 100))
      : 0;
  ...
  completions.forEach((completion) => {
    ...
    if (
      ...
      statusMap.set(key, completion.status);
      return;
    }

    if (completion.value === null && current === undefined) {
      statusMap.set(key, completion.status);
    }
  });
